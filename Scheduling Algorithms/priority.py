...
def priority_scheduling(processes):
    ...
    while completed_processes < n:
        ...
        if available_processes:
            # Select process with the highest priority (assuming lower value = higher priority)
...
def main():
    # User input for process IDs, burst times, arrival times, and priorities
    ...
    priorities = list(map(int, input("Enter priorities as space-separated values (lower = higher priority): ").split()))
    
    n = len(process_ids)
    processes = [Process(process_ids[i], burst_times[i], arrival_times[i], priorities[i]) for i in range(n)]

    # Perform Priority scheduling
    gantt_chart, avg_waiting_time, avg_turnaround_time = priority_scheduling(processes)
